...
async def merge(
    ...
    # ── Atualiza aba Contagem (100% DINÂMICA) ─────────────────────────────────
    if "Contagem" in wb_cons.sheetnames:
        ...
        if unidades_para_adicionar:
            # O Python descobre sozinho quantos materiais compõem o bloco de cada unidade
            template_unidade = str(ws_cont.cell(row=2, column=1).value or "")
            qtd_linhas_template = 0
            for r in range(2, 20):  # Lê o primeiro bloco como molde
                if r > 2 and ws_cont.cell(row=r, column=1).value:
                    break # Começou outra unidade, para de contar
                qtd_linhas_template += 1

            if qtd_linhas_template == 0:
                qtd_linhas_template = 3 # Fallback de segurança

            template_rows = {}
            for offset in range(qtd_linhas_template):
            ...
            rows_inseridos = len(unidades_para_adicionar) * qtd_linhas_template
            ...
            for unidade in unidades_para_adicionar:
                for offset in range(qtd_linhas_template):
                    ...
                    for col in range(1, ws_cont.max_column + 1):
                        ...
                        if col == 1:
                            ...
                            new_f = re.sub(r"=SUM\([A-Z]\d+:[A-Z]\d+\)", f"=SUM(C{rn}:I{rn})", new_f)
                            cell.value = new_f
                        
                        # Copia a estilização (cores e bordas)
                        if col <= len(ref_cont):
                ...
                current_row += qtd_linhas_template
            ...
            for r in range(1, ws_cont.max_row + 1):
                ...
                if not val.startswith("=SUM"): continue
                m_sum = re.match(r"=SUM\(C(\d+):I(\d+)\)(.*)", val)
                if m_sum and int(m_sum.group(1)) != r:
                    cell_k.value = f"=SUM(C{r}:I{r}){m_sum.group(3)}"

    # ── Atualiza Resumo Geral (100% DINÂMICO) ─────────────────────────────────
    if "Resumo Geral" in wb_cons.sheetnames and "Contagem" in wb_cons.sheetnames:
        ws_cont = wb_cons["Contagem"]
        ws_res  = wb_cons["Resumo Geral"]

        # 1. Mapeia em quais linhas cada material está na aba Contagem
        mat_rows = {}
        for i in range(1, ws_cont.max_row + 1):
            mat = str(ws_cont.cell(row=i, column=2).value or "").strip()
            if mat:
                if mat not in mat_rows:
                    mat_rows[mat] = []
                mat_rows[mat].append(i)

        def build_sum(rows, cont_col):
            if not rows: return ""
            return "=" + "+".join(f"Contagem!{cont_col}{r}" for r in rows)

        # 2. Procura os materiais na aba Resumo Geral e cria a fórmula automaticamente
        # (Assumindo que os nomes dos materiais estão na Coluna 1 ou 2 do Resumo)
        for r_res in range(1, ws_res.max_row + 1):
            nome_resumo = str(ws_res.cell(row=r_res, column=1).value or "").strip()
            if not nome_resumo: continue
            
            linhas_do_material = []
            for mat_contagem, linhas in mat_rows.items():
                # Busca flexível (Ex: "CAMISA UV" encontra "Camisa U.V para GV")
                if nome_resumo.upper() in mat_contagem.upper() or mat_contagem.upper() in nome_resumo.upper():
                    linhas_do_material = linhas
                    break
            
            if linhas_do_material:
                # O Python injeta as fórmulas de C até J automaticamente
                colunas_tamanhos = ["C","D","E","F","G","H","I","J"]
                for idx, cont_col in enumerate(colunas_tamanhos):
                    ws_res.cell(row=r_res, column=idx+2).value = build_sum(linhas_do_material, cont_col)
                # O total geral do material vai para a coluna K
                ws_res.cell(row=r_res, column=10).value = build_sum(linhas_do_material, "K")
